...
installed_version: typing.Tuple[int, int, int] = version2tuple(VERSION)

PY_VERSION: str = ".".join(str(e) for e in sys.version_info[:3])
...
class VersionChecker(BotPlugin):
    ...
    def _get_version(self) -> str:
        ...
        version: str = VERSION
        major_py_version: str = PY_VERSION.partition(".")[0]
    ...
    def _async_vcheck(self) -> None:
        current_version_txt: str = self._get_version()
        self.log.debug("Installed Errbot version is: %s", current_version_txt)
        current_version: typing.Tuple[int, int, int] = version2tuple(current_version_txt)
