...
```python
        ...
        except (requests.exceptions.HTTPError, requests.exceptions.ConnectionError, json.decoder.JSONDecodeError) as e:
            self.log.info(f"Could not establish connection to retrieve latest version due to error: {e}")
```
        return version
